...
public:
    ...
    virtual double Evaluate(std::function<double(Position)> callback_func) const = 0;
...
public:
    ...
    double Evaluate(std::function<double(Position)> callback_func) const override {
        double result = 0;
        double lhs = lhs_->Evaluate(callback_func);
        double rhs = rhs_->Evaluate(callback_func);
        if (type_ == Add) {
            result  = lhs + rhs;
...
public:
    ...
    double Evaluate(std::function<double(Position)> callback_func) const override {
        if (type_ == UnaryPlus) {
            return operand_->Evaluate(callback_func);
...
FormulaAST ParseFormulaAST(std::string_view in_str) {
    std::istringstream in((std::string(in_str)));
...
double FormulaAST::Execute(std::function<double(Position)> callback_func) const {
    return root_expr_->Evaluate(std::move(callback_func));
}
